...
final class LocalBackupTests: XCTestCase {
    ...
    func testRoundTripPreviewRestorePauseAndUndoPreserveUnrelatedData() throws {
        try withContext { defaults, directory in
            ...
            defaults.set(Data("PRIVATE LOCAL PATHS".utf8), forKey: FileShelfStore.locationsKey)
            ...
            let backup = try LocalBackupDocument.decode(Data(contentsOf: file))
            XCTAssertNil(backup.values[FileShelfStore.bookmarkKey])
