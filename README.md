...
- 展开刘海在「文件」后新增「便笺」入口，可直接编辑自动保存的本地便笺，并支持 `⌘4` 快速切换。
- 私密收藏停留当前页面时保持解锁；离开页面、切换应用或窗口、锁屏和休眠时仍会立即隐藏。
- 身份验证及钥匙串访问后等待原窗口稳定恢复焦点，必要时提供明确的钥匙串恢复入口和错误码。
- 小组件预先保存本月和下月日程，主应用在月末未运行时，跨月后仍能显示下月安排。
