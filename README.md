...
在展开刘海中加入可选的**文件浮窗**。选择一个常用文件夹后，可直接浏览目录、打开文件、在访达中定位、复制路径，或把文件拖到其他应用。访达文件拖到紧凑刘海会打开文件页，但不会移动或复制原文件。

文件浮窗默认关闭，可在设置中启用并选择目录。目录 bookmark 只保存在本机且不进入备份；文件访问被限制在所选根目录内。原有日历、会议和专注入口保持不变。

[下载 1.3.0](https://github.com/GpsLypy/NotchCalendar/releases/tag/v1.3.0) · [本版变化与验证](docs/quality/RELEASE_1.3.0.md)。
