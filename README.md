...
- 首次启动直接进入工作台，自动开始 7 天试用，不先弹出付款窗口。
- 顶部买断提示可关闭，侧栏显示试用剩余天数；到期后保留本地数据，提供激活入口。
- 自愿打赏不等于购买授权；¥9.9 永久买断仍需联系作者领取授权码。
...
采用闭源发行。新用户首次启动自动开始 **7 天免费试用**，不先展示收款二维码；试用结束后需要永久授权码。一次支付 **¥9.9**，永久使用，无订阅。已激活用户不再自动弹出收款窗口。

历史 MIT 版本不受新的付费规则追溯影响。请以所下载版本的发行说明为准。

## 支持作者与永久激活

1. 在工作台侧栏「授权与试用」（或设置中的同名入口）扫码支付 ¥9.9。
