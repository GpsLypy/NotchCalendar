...
在展开刘海中加入可选的**文件浮窗**。最多可收藏 12 个常用文件夹，通过 Finder 式侧栏、文件列和目录树浏览；选中文件后可直接打开、复制、在访达中定位或复制路径，也可把文件拖到其他应用。访达文件拖到紧凑刘海会打开文件页，但不会移动或复制原文件。

文件浮窗默认关闭，可在设置中启用、添加和排序收藏目录。目录 bookmark 只保存在本机且不进入备份；文件访问被限制在当前收藏的根目录内。原有日历、会议和专注入口保持不变。

[下载 1.4.1](https://github.com/GpsLypy/NotchCalendar/releases/tag/v1.4.1) · [本版变化与验证](docs/quality/RELEASE_1.4.1.md)。

本版仅使用 ad-hoc 签名，手动安装。最终性能与部分实机验收仍待完成，具体范围见[质量报告](docs/quality/README.md)。

### 刘海日历与会议助手

平时保持紧凑，悬停或点击后展开日程；悬停响应支持快速、平衡和防误触三档，也可以切换为仅点击打开。完整桌面工作台提供今日总览、月历和工具侧栏，无刘海的显示器也能使用顶部居中入口。
