...
在展开刘海中加入可选的**文件浮窗**。最多可收藏 12 个常用文件夹，通过 Finder 式侧栏、文件列和目录树浏览；可打开文件、在访达中定位、复制路径，或把文件拖到其他应用。访达文件拖到紧凑刘海会打开文件页，但不会移动或复制原文件。

文件浮窗默认关闭，可在设置中启用、添加和排序收藏目录。目录 bookmark 只保存在本机且不进入备份；文件访问被限制在当前收藏的根目录内。原有日历、会议和专注入口保持不变。

[下载 1.4.0](https://github.com/GpsLypy/NotchCalendar/releases/tag/v1.4.0) · [本版变化与验证](docs/quality/RELEASE_1.4.0.md)。
