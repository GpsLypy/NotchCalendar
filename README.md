...
- 试用到期后仍可在只读「我的数据」查看本地便笺、专注记录和文件位置，并分别导出普通备份与需要验证身份的私密收藏。
- 支持隐藏侧栏，使用 Option-Command-S 或紧凑导航恢复访问。
- 七天试用最后 48 小时仅提醒一次；付款码按需展开，打赏与永久买断明确区分。
...
采用闭源发行。新用户首次启动自动开始 **7 天免费试用**，不先展示收款二维码；试用结束后可只读查看及导出本地数据，继续使用其他功能需要永久授权码。一次支付 **¥9.9**，永久使用，包含未来版本更新，无订阅。已激活用户不再自动弹出收款窗口。

历史 MIT 版本不受新的付费规则追溯影响。请以所下载版本的发行说明为准。

## 支持作者与永久激活

1. 在工作台侧栏「授权与试用」（或设置中的同名入口）主动展开收款二维码并支付 ¥9.9。
